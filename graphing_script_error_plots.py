...
def plot_position_error(df: pd.DataFrame, out_dir: str):
    ...
    txt = (
        rf"$p_x$  RMSE={_rmse(ex):.4f}m" "\n"
        rf"$p_y$  RMSE={_rmse(ey):.4f}m" "\n"
        rf"$p_z$  RMSE={_rmse(ez):.4f}m"
    )
    ax.text(0.62, 0.28, txt, transform=ax.transAxes, fontsize=11)

    _save_fig(fig, os.path.join(out_dir, "position_error.svg"))


def plot_linear_velocity_error(df: pd.DataFrame, out_dir: str):
    ...
    txt = (
        rf"$v_{{Dx}}$  RMSE={_rmse(evx):.4f}m/s" "\n"
        rf"$v_{{Dy}}$  RMSE={_rmse(evy):.4f}m/s" "\n"
        rf"$v_{{Dz}}$  RMSE={_rmse(evz):.4f}m/s"
    )
    ax.text(0.66, 0.30, txt, transform=ax.transAxes, fontsize=11)

    _save_fig(fig, os.path.join(out_dir, "linear_velocity_error.svg"))


def plot_angular_velocity_error(df: pd.DataFrame, out_dir: str):
    ...
    txt = (
        rf"$\Omega_x$  RMSE={_rmse(ewx):.4f} rad/s" "\n"
        rf"$\Omega_y$  RMSE={_rmse(ewy):.4f} rad/s" "\n"
        rf"$\Omega_z$  RMSE={_rmse(ewz):.4f} rad/s"
    )
    ax.text(0.62, 0.28, txt, transform=ax.transAxes, fontsize=11)

    _save_fig(fig, os.path.join(out_dir, "angular_velocity_error.svg"))


def plot_orientation_error(df: pd.DataFrame, out_dir: str):
    e = df["estimate_error"]
    ...
    ax.text(0.70, 0.90, f"RMSE={_rmse(e):.2f} deg", transform=ax.transAxes, fontsize=11)

    _save_fig(fig, os.path.join(out_dir, "orientation_error.svg"))


def plot_pz_comparison(df: pd.DataFrame, out_dir: str):
    fig, ax = plt.subplots(figsize=(7, 5))
    ...
    _save_fig(fig, os.path.join(out_dir, "pz_comparison.svg"))


def plot_vdy_comparison(df: pd.DataFrame, out_dir: str):
    fig, ax = plt.subplots(figsize=(7, 5))
    ...
    _save_fig(fig, os.path.join(out_dir, "vdy_comparison.svg"))
...
def plot_omegaz_comparison(df: pd.DataFrame, out_dir: str):
    ...
    _save_fig(fig, os.path.join(out_dir, "omegaz_comparison.svg"))


def plot_vertex_p1_components(df: pd.DataFrame, out_dir: str):
    fig, ax = plt.subplots(figsize=(7, 5))
    ...
    _save_fig(fig, os.path.join(out_dir, "vertex_p1_components.svg"))


def plot_qw_comparison(df: pd.DataFrame, out_dir: str):
    fig, ax = plt.subplots(figsize=(7, 5))
    ...
    _save_fig(fig, os.path.join(out_dir, "qw_comparison.svg"))


def plot_qz_comparison(df: pd.DataFrame, out_dir: str):
    fig, ax = plt.subplots(figsize=(7, 5))
    ...
    _save_fig(fig, os.path.join(out_dir, "qz_comparison.svg"))
...
def plot_size_estimation(
    ...
    ax.text(1.01, true_L, f"{true_L:.1f}m", transform=ax.get_yaxis_transform(),
            va="center", fontsize=11)
    ax.text(1.01, true_W, f"{true_W:.1f}m", transform=ax.get_yaxis_transform(),
            va="center", fontsize=11)
    ax.text(1.01, true_D, f"{true_D:.1f}m", transform=ax.get_yaxis_transform(),
            va="center", fontsize=11)

    _save_fig(fig, os.path.join(out_dir, "size_estimation.svg"))
...
def make_all_plots(
    ...
    plot_position_error(df, out_dir)
    plot_linear_velocity_error(df, out_dir)
    plot_angular_velocity_error(df, out_dir)
    plot_orientation_error(df, out_dir)
    plot_pz_comparison(df, out_dir)
    plot_vdy_comparison(df, out_dir)
    plot_omegaz_comparison(df, out_dir)
    plot_vertex_p1_components(df, out_dir)
    plot_qw_comparison(df, out_dir)
    plot_qz_comparison(df, out_dir)
    plot_size_estimation(df, out_dir, true_L=true_L, true_W=true_W, true_D=true_D)

    print(f"Saved plots to: {out_dir}")


if __name__ == "__main__":
    csv_path = r"results_of_ass_res_results_kompsat_200s__rpca_20__ortho_0p5__eig_0p24__brecal_30__run_061__sim_debris_trimesh_test_kompsat_27.csv"
    out_dir = r"figs"
