...
def run(pickle_file, configs, logger):
    ...
    q_true = np.array(get_true_orientation(Rot_L_to_B, omega_true, debris_pos, dt, rotm2quat(rodrigues(omega_L, np.deg2rad(initial_angle_rotation)))))
    ...
    for i in range(nframes):
        ...
        if i > 0:
            ...
            ran_pred_thresh = configs['ransac_prediction_threshold']
            pca_pred_thresh = configs['pca_prediction_threshold']
            ran_pca_thresh = configs['ransac_pca_threshold']
        ...
        choice = 'prediction'
        if i > 0:
            if pca_true_diff < configs['true_orientation_difference'] or ransac_true_diff < configs['true_orientation_difference']:
        ...
        perfect_metric_choices.append(choice)
        ...
        number_of_planess.append(num_planes)
        try:
            eig_x = evals[0]
        except IndexError:
            eig_x = 0
    ...
    master_file['number_of_planes'] = number_of_planess
    master_file['eig_x'] = eig_xs
    master_file['eig_y'] = eig_ys
    master_file['eig_z'] = eig_zs
    ...
    ##############
    if 'p' in configs['options']:
        logger.info("True Pred: " + str(true_pred))
        logger.info("True ran: " + str(true_ran))
        logger.info("True pca: " + str(true_pca))
        logger.info("Used Pred: " + str(prediction))
        logger.info("Used Ran: " + str(ransac))
        logger.info("Used PCA: " + str(pca))
        logger.info("PCA bins: " + str([pca1, pca2, pca3, pca4, pca5, pca6, pca7, pca8, pca9, pca10, pca11, pca12]))
        logger.info("Pred bins: " + str([pred1, pred2, pred3, pred4, pred5, pred6, pred7, pred8, pred9, pred10, pred11, pred12, pred13]))
        logger.info("Ran bins: " + str([ran1, ran2, ran3, ran4, ran5, ran6, ran7, ran8]))
    ...
    results = [true_pred, true_ran, true_pca, prediction, ransac, pca, pca1, pca2, pca3, pca4, pca5, pca6, pca7, pca8,
               pca9, pca10, pca11, pca12, pred1, pred2, pred3, pred4, pred5, pred6, pred7, pred8, pred9, pred10, pred11,
               pred12, pred13, ran1, ran2, ran3, ran4, ran5, ran6, ran7, ran8, rmse_px, rmse_py, rmse_pz, rmse_omx,
               rmse_omy, rmse_omz, rmse_vdx, rmse_vdy, rmse_vdz, rmse_x_before, rmse_y_before, rmse_z_before,
