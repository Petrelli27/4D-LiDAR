...
def F_matrix(dt, R, p_k, p1_k, p2_k, p3_k, p4_k, p5_k, p6_k, p7_k, p8_k, omega_k, q_k):
    ...
    F[33:, 6:9] = 0.5 * dt * np.concatenate((-np.array([q_123k]), bottom), axis=0)
...
real_initialization = True  # set to False to use values below for initialization
...
for i in range(nframes):
    ...
    if i == 0:
        if real_initialization:  # initialize from initial measurement
            #######################
            ...
            z_pi_0, p_0, R_0 = boundingbox.bbox3d(X_i, Y_i, Z_i, return_evec=True)

            z_omega_0_B = estimate(XBs[i], YBs[i], ZBs[i], Rot_L_to_B[i] @ p_0, Rot_L_to_B[i] @ v_0, VBs[i])
            z_omega_0 = Rot_B_to_L[i] @ z_omega_0_B

            prev_box_B = (Rot_L_to_B[i] @ z_pi_0).T
            omega_L_to_B = np.array([0, 0, 0])

            q_0 = rotm2quat(R_0)
            x_0 = np.array([p_0, v_0, z_omega_0, z_pi_0[:, 0],  z_pi_0[:, 1],  z_pi_0[:, 2],  z_pi_0[:, 3],  z_pi_0[:, 4],  z_pi_0[:, 5],  z_pi_0[:, 6],  z_pi_0[:, 7]]).ravel()  # Initial State vector
            x_0 = np.append(x_0, q_0.ravel())

        else:  # initialize from preselected values
            ...
            x_0 = np.array(
                [p_0, v_0, omega_0, p1_0, p2_0, p3_0, p4_0, p5_0, p6_0, p7_0, p8_0]).ravel()  # Initial State vector
            x_0 = np.append(x_0, q_0.ravel())


        ###############
        # common to either initialization scheme
        #############

        P_0 = np.diag(
            [0.25, 0.5, 0.25, 0.05, 0.05, 0.05, 0.01, 0.01, 0.01, 0.25, 0.5, 0.25, 0.25, 0.5, 0.25, 0.25, 0.5, 0.25,
             0.25, 0.5, 0.25, 0.25, 0.5, 0.25, 0.25, 0.5, 0.25, 0.25, 0.5, 0.25, 0.25, 0.5, 0.25,
             0.05, 0.05, 0.05, 0.05])  # Initial Covariance matrix
        ...
        qq = 0.00005
        Q = np.diag(
            [qpxyz, qpxyz, qpxyz, qv, qv, qv, qom, qom, qom, qpixz, qpyz, qpixz, qpixz, qpyz, qpixz, qpixz, qpyz, qpixz,
             qpixz, qpyz, qpixz, qpixz, qpyz, qpixz, qpixz, qpyz, qpixz, qpixz, qpyz, qpixz, qpixz, qpyz, qpixz, qq, qq, qq, qq])
        ...
        # Compute Jacobian
        F_kp1 = F_matrix(dt, R_k_kp1, p_k, p1_k, p2_k, p3_k, p4_k, p5_k, p6_k, p7_k, p8_k, omega_k, q_k)
        ...
        x_kp1 = np.append(x_kp1, q_kp1.ravel())
        ...
        z_p1_k, z_p2_k, z_p3_k, z_p4_k, z_p5_k, z_p6_k, z_p7_k, z_p8_k = nearest_search(pi_pk1 - p_k - v_k * dt, z_pi_k,
                                                                                        z_p_k)
        ...
        if i > 300 and abs(np.linalg.norm(z_p_k - p_kp1)) > 0.3:
            ...
            x_kp1[33:] = normalize_quat(x_kp1[33:])
...
fig = plt.figure()
plt.plot(np.arange(0, dt * nframes, dt), x_s[:m1 - 1, 0] - debris_pos[:, 0], label='Error $\displaystyle p_x$',
         linewidth=2)
plt.plot(np.arange(0, dt * nframes, dt), x_s[:m1 - 1, 1] - debris_pos[:, 1], label='Error $\displaystyle p_y$',
         linewidth=2)
plt.plot(np.arange(0, dt * nframes, dt), x_s[:m1 - 1, 2] - debris_pos[:, 2], label='Error $\displaystyle p_x$',
         linewidth=2)
...
fig = plt.figure()
plt.plot(np.arange(0, dt * nframes, dt), z_v_s[:, 0] - debris_vel[:, 0], label='Error $\displaystyle v_{Tx}$',
         linewidth=1)
plt.plot(np.arange(0, dt * nframes, dt), z_v_s[:, 1] - debris_vel[:, 1], label='Error $\displaystyle v_{Ty}$',
         linewidth=1)
plt.plot(np.arange(0, dt * nframes, dt), z_v_s[:, 2] - debris_vel[:, 2], label='Error $\displaystyle v_{Tz}$',
         linewidth=1)
