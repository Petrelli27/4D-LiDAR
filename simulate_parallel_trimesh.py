...
def process_frame(i, debris_file, debris_pos, debris_vel, omega_L, dt, r0, rdot0, omeg, res_box, ang_res):
    ...
    if i == 0:
        ...
        Rot_4by4 = np.eye(4)
        Rot_4by4[:3,:3] = Rot_to_B
        # debris.rotate_using_matrix(Rot_to_B.T)

        # use trimesh instead of stl mesh rotation
        ...
        debris.apply_transform(trimesh.transformations.rotation_matrix(angle, axis))
...
if __name__ == '__main__':
    ...
    data.append(omega_L)
    data.append(dt)
    data.append(angle_0) # in degrees
    with open('sim_kompsat_trimesh_test.pickle', 'wb') as sim_data:
