...
def gram_schmidt(vectors):
    ...
    # print(len(basis))
    if len(basis) < 3:
        print('RANSAC Error: not enough basis vectors')
...
def boundingbox3D_RANSAC(x, y, z, q_kp1, return_evec=False, visualize=False):
    ...
    # Parameters
    distance_threshold = 0.05  # Adjust based on your data
    ransac_n = 3
    num_iterations = 300
    ...
    while len(remaining_points.points) > min_inliers:
        # Apply RANSAC to segment a plane
        o3d.utility.random.seed(42)  # not available in v0.9.0
    ...
    num_planes = len(normal_vecs)

    # Visualize all detected planes
    all_planes_colors = [[1, 0, 0], [0, 1, 0], [0, 0, 1], [1, 0, 1], [1, 1, 0],
                         [0, 1, 1]]  # Different colors for planes
    ...
    if len(projections) < 3:  # not enough basis vectors
        if return_evec:
            return [], [], np.array([]), [], [], num_planes
        else:
            return [], [], [], [], num_planes
    else:
        ...
        rrc = np.matmul(projections.T,
                        rectCoords(xmin, ymin, zmin, xmax, ymax, zmax))  # rrc = rotated rectangle coordinates
        ...
        if return_evec:
            return rrc, [c_x, c_y, c_z], projections.T, sorted_vectors, ranking, num_planes
        else:
            return rrc, [c_x, c_y, c_z], sorted_vectors, ranking, num_planes



from scipy.optimize import linear_sum_assignment

def boundingbox3D_RANSAC_new(x, y, z, q_kp1, return_evec=False, visualize=False):
    """
    Plane-segment point cloud, rank plane normals using:
      - alignment to predicted orientation
      - plane support (number of inliers)
      - plane fit quality (mean residual)
    then assign detected planes uniquely to predicted body axes.

    Notes:
    - Plane normals are sign ambiguous, so angle comparisons use abs(dot).
    - A one-to-one assignment is done with the Hungarian algorithm.
    """
    ...
    def angle_between_plane_normals(a, b):
        """
        Plane-normal angle with sign ambiguity removed.
        ...
        if au is None or bu is None:
            return np.pi / 2
        c = np.clip(np.abs(np.dot(au, bu)), -1.0, 1.0)
        return np.arccos(c)

    def angle_axis_to_plane_normal(axis, normal):
        """
        Angle between predicted axis and plane normal, with sign ambiguity removed.
        """
        au = safe_unit(axis)
        nu = safe_unit(normal)
        if au is None or nu is None:
            return np.pi / 2
        c = np.clip(np.abs(np.dot(au, nu)), -1.0, 1.0)
        return np.arccos(c)
    ...
    # Raise this from 3 if possible; 3 is too permissive
    # min_inliers = max(20, int(0.01 * len(points))) if len(points) > 0 else 20
    min_inliers = 20
    all_planes = []
    ...
    while len(remaining_points.points) >= min_inliers:
        ...
        # Mean absolute point-to-plane residual
        normal_norm = np.linalg.norm(normal)
        if normal_norm < 1e-12:
            remaining_points = remaining_points.select_by_index(inliers, invert=True)
            continue

        residuals = np.abs(inlier_pts @ normal + d) / normal_norm
        ...
        all_planes.append(inlier_cloud)
        all_points = np.vstack((all_points, inlier_pts))

        remaining_points = remaining_points.select_by_index(inliers, invert=True)

    num_planes = len(plane_infos)
    ...
    for idx, plane in enumerate(all_planes):
        plane.paint_uniform_color(all_planes_colors[idx % len(all_planes_colors)])
    ...
    # Need at least 2 planes to have any hope
    if num_planes < 2:
        if return_evec:
            return [], [], np.array([]), [], [], num_planes
        else:
    ...
    # -------------------------
    # Build ranking / cost matrix
    # -------------------------
    normal_vecs = np.array([p["normal"] for p in plane_infos])   # shape (M,3)
    inlier_counts = np.array([p["num_inliers"] for p in plane_infos], dtype=float)
    residuals = np.array([p["mean_residual"] for p in plane_infos], dtype=float)

    # Normalize support and residual for scoring
    inlier_score = inlier_counts / max(np.max(inlier_counts), 1.0)

    if np.max(residuals) > np.min(residuals):
        residual_score = (residuals - np.min(residuals)) / (np.max(residuals) - np.min(residuals))
    else:
        residual_score = np.zeros_like(residuals)

    # Predicted orientation
    R = quat2rotm(q_kp1)
    pred_axes = R.T  # same convention as your original code

    # cost[i, j] = cost for assigning predicted axis i to detected plane j
    # lower is better
    #
    # weights:
    #   angle term dominates
    #   residual penalizes poor plane fits
    #   inlier support rewards large/important planes
    w_angle = 1.0
    w_resid = 0.35
    w_inlier = 0.35

    cost = np.zeros((3, num_planes), dtype=float)

    for i, axis in enumerate(pred_axes):
        for j, normal in enumerate(normal_vecs):
            ang = angle_axis_to_plane_normal(axis, normal)
            cost[i, j] = (
                w_angle * ang
                + w_resid * residual_score[j]
                - w_inlier * inlier_score[j]
            )

    # -------------------------
    # Unique assignment
    # -------------------------
    # If there are fewer than 3 planes, solve on available planes and later complete basis
    n_assign = min(3, num_planes)
    row_ind, col_ind = linear_sum_assignment(cost[:, :])  # works for rectangular matrices

    # Keep rows ordered by predicted axis index
    order = np.argsort(row_ind)
    row_ind = row_ind[order]
    col_ind = col_ind[order]

    assigned_normals = normal_vecs[col_ind].copy()

    # Flip signs so they are consistent with predicted axes
    for k, axis_idx in enumerate(row_ind):
        if np.dot(pred_axes[axis_idx], assigned_normals[k]) < 0:
            assigned_normals[k] *= -1.0

    # -------------------------
    # Complete basis if needed
    # -------------------------
    # If only 2 unique normals are available, use cross product for the third.
    # If 3 are available, Gram-Schmidt will orthonormalize.
    if assigned_normals.shape[0] == 2:
        v3 = np.cross(assigned_normals[0], assigned_normals[1])
        v3u = safe_unit(v3)
        if v3u is not None:
            assigned_normals = np.vstack((assigned_normals, v3u))

    # If more than 3 somehow sneak in, keep first 3
    assigned_normals = assigned_normals[:3]

    projections = gram_schmidt(assigned_normals)

    if len(projections) < 3:
        if return_evec:
            return [], [], np.array([]), assigned_normals, cost, num_planes
        else:
            return [], [], assigned_normals, cost, num_planes

    # -------------------------
    # Bounding box in aligned frame
    # -------------------------
    points_used = all_points[1:, :]
    means = np.mean(points_used, axis=0)
    centered_data = (points_used - means).T
    ...
    rrc = np.matmul(
        projections.T,
        rectCoords(xmin, ymin, zmin, xmax, ymax, zmax)
    )
    ...
    if visualize:
        ...
        # Assigned raw normals
        for k in range(min(3, len(assigned_normals))):
            ax.plot([0, assigned_normals[k, 0]],
                    [0, assigned_normals[k, 1]],
                    [0, assigned_normals[k, 2]], color='blue')

        ax.scatter(centered_data[0, :], centered_data[1, :], centered_data[2, :], color='black', s=1)

        associatedBbox_2 = (rrc.T - means).T
        drawrectangle(
            ax,
            associatedBbox_2[:, 0], associatedBbox_2[:, 1], associatedBbox_2[:, 2], associatedBbox_2[:, 3],
            associatedBbox_2[:, 4], associatedBbox_2[:, 5], associatedBbox_2[:, 6], associatedBbox_2[:, 7],
            'orange', 2
        )
        plt.show()
    ...
    if return_evec:
        return rrc, [c_x, c_y, c_z], projections.T, assigned_normals, cost, num_planes
    else:
        return rrc, [c_x, c_y, c_z], assigned_normals, cost, num_planes
